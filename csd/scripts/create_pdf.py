...
def main(args):
    html = []
    chars = []
    for entry in DBSession.query(Parameter).order_by(Parameter.name).limit(10):
        if entry.name[0] not in chars:
            html.append('<h2>%s</h2>' % entry.name[0])
            chars.append(entry.name[0])
        html.append('<h3>%s</h3>' % entry.name)
        adapter = get_adapter(IRepresentation, entry, args.env['request'], ext='snippet.html')
        html.append(adapter.render(entry, args.env['request']))
        html.append('<p class="separator">&nbsp;<p>')

    ttf = os.path.abspath(os.path.join(os.path.dirname(csd.__file__), 'static', 'charissil'))
    with open('test.pdf', 'wb') as fp:
        pisa.CreatePDF(html_tmpl % (
            css_tmpl.format(ttf),
            args.env['request'].dataset.name,
            '<h1>%s</h1><blockquote>edited by %s</blockquote>' % (
                args.env['request'].dataset.name,
                args.env['request'].dataset.formatted_editors(),
            ),
            ''.join(html)), dest=fp)
